...
l=length(t) %Finding the length of time
x1 = 1.*(t>=1 & t<=10); %Defining the signal x1(t)
l1 = length(x1) %Finding the length of signal x1(t)
x2 = 1.*(t>=2 & t<=10); %Defining the signal x2(t)
l2 = length(x2) %Finding the length of signal x2(t)
x3 = conv(x1,x2); %Finding the convolution of x1(t) & x2(t)
l3 = length(x3) %Finding the length of signal x3(t)
